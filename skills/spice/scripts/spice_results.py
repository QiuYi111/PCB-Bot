...
def build_report(simulation_runs):
    ...
    try:
        ...
        _local_kicad_scripts = _os.path.join(
            _os.path.dirname(_os.path.abspath(__file__)),
            '..', '..', 'kicad', 'scripts')
        _kicad_candidates = [_local_kicad_scripts]
        _configured_kicad = _os.environ.get("KICAD_SKILL_ROOT")
        if _configured_kicad:
            _kicad_candidates.append(_os.path.join(
                _os.path.expanduser(_configured_kicad), "scripts"))
        _kicad_candidates.extend([
            _os.path.expanduser("~/.codex/skills/kicad/scripts"),
            _os.path.expanduser("~/.agents/skills/kicad/scripts"),
        ])
        for _candidate in _kicad_candidates:
            if _os.path.isfile(_os.path.join(_candidate, "finding_schema.py")):
                _sys.path.insert(0, _os.path.abspath(_candidate))
                break
