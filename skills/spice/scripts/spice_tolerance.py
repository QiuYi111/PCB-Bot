...
# Allow importing from a repository-provided or installed KiCad Skill.
_local_kicad = os.path.abspath(os.path.join(
    os.path.dirname(__file__), "..", "..", "kicad", "scripts"))
_kicad_candidates = [_local_kicad]
_configured_kicad = os.environ.get("KICAD_SKILL_ROOT")
if _configured_kicad:
    _kicad_candidates.append(os.path.join(
        os.path.expanduser(_configured_kicad), "scripts"))
_kicad_candidates.extend([
    os.path.expanduser("~/.codex/skills/kicad/scripts"),
    os.path.expanduser("~/.agents/skills/kicad/scripts"),
])
for _candidate in _kicad_candidates:
    if os.path.isfile(os.path.join(_candidate, "kicad_utils.py")):
        sys.path.insert(0, _candidate)
        break
else:
    raise ImportError(
        "KiCad Skill scripts not found; set KICAD_SKILL_ROOT to the installed kicad Skill"
    )
