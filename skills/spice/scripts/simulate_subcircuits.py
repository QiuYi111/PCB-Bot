...
def _kicad_scripts_dir():
    """Find the installed or repository-provided KiCad helper scripts."""
    local = os.path.abspath(os.path.join(
        os.path.dirname(__file__), '..', '..', 'kicad', 'scripts'))
    candidates = [local]
    configured = os.environ.get("KICAD_SKILL_ROOT")
    if configured:
        candidates.append(os.path.join(os.path.expanduser(configured), "scripts"))
    candidates.extend([
        os.path.expanduser("~/.codex/skills/kicad/scripts"),
        os.path.expanduser("~/.agents/skills/kicad/scripts"),
    ])
    for candidate in candidates:
        if os.path.isfile(os.path.join(candidate, "analysis_cache.py")):
            return candidate
    return None
...
def main():
    ...
    if args.analysis_dir:
        ...
        kicad_scripts = _kicad_scripts_dir()
        if not kicad_scripts:
            print("Error: KiCad Skill scripts not found. Set KICAD_SKILL_ROOT to the installed kicad Skill.", file=sys.stderr)
            sys.exit(1)
